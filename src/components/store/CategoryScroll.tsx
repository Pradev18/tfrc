      ...
      <div className="container-pawmart">
        ...
        <div className="-mx-4 flex gap-3 overflow-x-auto px-4 pb-2 scrollbar-hide sm:mx-0 sm:grid sm:grid-cols-3 sm:gap-3 sm:overflow-visible sm:px-0 md:grid-cols-4 md:gap-4 lg:grid-cols-4 xl:grid-cols-8">
          {categories.map((cat) => (
            ...
            >
              <div className="glass-card store-glass-category relative aspect-square overflow-hidden rounded-2xl">
                {cat.imageUrl ? (
                  <Image
                    src={cat.imageUrl}
                    alt={cat.name}
                    fill
                    className="object-contain p-3.5 transition-transform duration-500 ease-out group-hover:scale-[1.06]"
